import { categoryName, favoriteIds, mapsSearchUrl, venueIcon } from '../../utils/storage.js';
...
function badge(venue) {
  ...
  if (venue.halalStatus === 'needs_verification') return <span className="badge unverified">! Needs verification</span>;
...
export default function VenueCard({ venue, onOpen, onFavorite }) {
  ...
  return <article className={`${styles.root} venue`} onClick={() => onOpen(venue)}><div className="venue-thumb">{venue.image ? <img src={venue.image} alt={`รูป ${venue.name}`} loading="lazy" /> : <span>{venueIcon(venue)}</span>}</div><div className="venue-body"><div>{badge(venue)}</div><h2>{venue.name || 'ไม่ระบุชื่อสถานที่'}</h2><span className="meta">{categoryName(venue)}{venue.address ? ` · ${venue.address}` : ''}</span>{venue.cuisine && <span className="meta">{venue.cuisine}</span>}<a className="venue-map-link" href={mapsSearchUrl(venue)} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>เปิดใน Google Maps ↗</a></div><button className={`heart ${saved ? 'saved' : ''}`} onClick={(event) => { event.stopPropagation(); onFavorite(venue.id); }} aria-label={saved ? 'ลบจากรายการโปรด' : 'บันทึกรายการโปรด'}>{saved ? '♥' : '♡'}</button></article>;
}
